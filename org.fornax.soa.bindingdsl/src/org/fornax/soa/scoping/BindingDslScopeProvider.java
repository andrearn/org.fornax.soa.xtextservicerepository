 ...
 *
 */
public class BindingDslScopeProvider extends ImportedNamespaceAwareLocalScopeProvider {
	...
	IScope scope_ServiceRef_service (ServiceRef ctx, final EReference ref) {
		...
		filter = createVersionFilter(v, ctx);
		return getScope(ctx, ref, filter);
	}

	private VersionFilter createVersionFilter(final VersionRef v, ServiceRef ctx) {
		...
		LifecycleState ownerState = getLifecycleState(ctx);
		LifecycleState minDevState = LifecycleState.PROPOSED;
		LifecycleState minTestState = LifecycleState.TEST;
		LifecycleState minProdState = LifecycleState.PRODUCTIVE;
	...
	protected Object getKey(Notifier context, EReference reference) {
		return Tuples.create(BindingDslScopeProvider.class, context, reference);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected Object getKey(Notifier context, EReference reference, VersionFilter filter) {
		List elements = new ArrayList();
		elements.add (BindingDslScopeProvider.class);
	...
	private LifecycleState getLifecycleState (EObject o) {
		if (o instanceof Environment)
			return getLifecycleStateForEnvironment((Environment) o);
		if (o instanceof DomainBinding)
			return getLifecycleStateForEnvironment(((DomainBinding) o).getEnvironment());
		if (o instanceof ServiceRef)
			return getLifecycleStateForEnvironment(((ServiceBinding)o.eContainer()).getEnvironment());
		if (o instanceof OperationRef) {
			OperationBinding opBind = (OperationBinding) o.eContainer();
			ServiceBinding svcBind = (ServiceBinding) opBind.eContainer();
			return getLifecycleStateForEnvironment(svcBind.getEnvironment());
		}
		return LifecycleState.DEVELOPMENT;
	}
	
	private LifecycleState getLifecycleStateForEnvironment (Environment env) {
		if (env.getType() == EnvironmentType.PROD)
			return LifecycleState.PRODUCTIVE;
		if (env.getType() == EnvironmentType.PRE_PROD)
			return LifecycleState.TEST;
		if (env.getType() == EnvironmentType.TEST)
			return LifecycleState.TEST;
		return LifecycleState.DEVELOPMENT;
	}
