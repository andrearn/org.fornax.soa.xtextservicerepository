...
import org.xkonnex.repo.dsl.basedsl.search.SearchPattern;
import org.xkonnex.repo.dsl.basedsl.ui.editor.ElementSelectionConverter;
...
public abstract class AbstractInformationControl extends PopupDialog implements IInformationControl, IInformationControlExtension, IInformationControlExtension2, DisposeListener {
	...
	@Inject
	protected XtextEditorSelectionStrategy editorSelectionStrategy;
	
	@Inject
	protected ElementSelectionConverter selectionConverter;
	...
	protected class NamePatternFilter extends ViewerFilter {
		...
		private boolean hasUnfilteredChild(TreeViewer viewer, Object element) {
			if (element instanceof EObject && !((EObject)element).eContents().isEmpty()) {
	...
	protected SearchPattern fStringMatcher;
...
//	private CustomFiltersActionGroup fCustomFiltersActionGroup;
	 ...
	 * @since 3.5
	 */
	protected IEObjectDescription fInitiallySelectedType;
...
//		fCustomFiltersActionGroup= new CustomFiltersActionGroup(getId(), fTreeViewer);
	...
	protected void setMatcherString(String pattern, boolean update) {
		if (pattern.length() == 0) {
			...
			int searchMode = SearchPattern.RULE_PATTERN_MATCH;
			if (!ignoreCase)
				searchMode = SearchPattern.RULE_CASE_SENSITIVE;
			fStringMatcher= new SearchPattern (searchMode);
	...
	private void gotoSelectedElement() {
		...
		if (selectedElement != null) {
			...
			if (selectedElement instanceof IEObjectDescription) {
				IEObjectDescription ieDesc = (IEObjectDescription) selectedElement;
				URI uri = ieDesc.getEObjectURI();
	...
	private TreeItem findElement(TreeItem[] items, TreeItem[] toBeSkipped, boolean allowToGoUp) {
		...
		for (int i= 0; i < items.length; i++) {
			...
			IEObjectDescription element= (IEObjectDescription)item.getData();
			if (element != null) {
...
//		fCustomFiltersActionGroup.fillViewMenu(viewMenu);
	...
	protected void inputChanged(Object newInput, Object newSelection) {
		...
		//FIXME selection
		if (newSelection instanceof IEObjectDescription) {
			IEObjectDescription javaElement= ((IEObjectDescription)newSelection);
//			if (javaElement.getElementType() == IJavaElement.TYPE)
				fInitiallySelectedType= (IEObjectDescription)javaElement;
//			else
//				fInitiallySelectedType= (IType)javaElement.getAncestor(IJavaElement.TYPE);
	...
	protected IDialogSettings getDialogSettings() {
		//FIXME get plugin settings or create new section
