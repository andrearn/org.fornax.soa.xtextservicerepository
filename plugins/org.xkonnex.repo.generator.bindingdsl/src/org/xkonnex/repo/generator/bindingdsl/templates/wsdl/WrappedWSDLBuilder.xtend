...
class WrappedWSDLBuilder {
	...
	*/
	def dispatch void toWrappedWSDL (SubNamespace ns, Profile profileName, Environment targetEnvironment) {
		ns.services.filter (s|s.isEligibleForEnvironment (targetEnvironment))
			.forEach (e|e.toWrappedWSDL (profileName, targetEnvironment));
	}
	
	def dispatch void toWrappedWSDL (SubNamespace ns, List<Profile> profiles, String profileName, List<Environment> environments, String targetEnvironment) {
	...
	def dispatch void toWrappedWSDL (Service svc, List<Profile> profiles, String profileName, List<Environment> environments, String targetEnvironment) {
		val env = environments.findFirst (e|e.name==targetEnvironment);
		svc.toWrappedWSDL (profiles.findFirst (e|e.name==profileName), env);
	}
	
	
	def dispatch void toWrappedWSDL (Service svc, Profile profile, Environment environment) {
