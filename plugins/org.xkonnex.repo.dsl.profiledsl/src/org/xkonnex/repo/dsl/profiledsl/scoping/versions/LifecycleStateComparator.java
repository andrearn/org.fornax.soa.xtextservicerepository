...
public class LifecycleStateComparator implements Comparator<LifecycleState> {
	...
	public int compare (LifecycleState s1, LifecycleState s2) {
		...
		if (s1.isIsInitial () && !s2.isIsInitial ())
			return -1;
		if (!s1.isIsInitial () && s2.isIsInitial () && !s2.isIsEnd ())
			return 1;
		if (s1.isIsEnd () && !s2.isIsEnd ())
			return 1;
		if (!s1.isIsEnd () && s2.isIsEnd ())
			return -1;
...
//				if (leftEnvTypes.size() == 0)
//					return 0;
//				if (leftEnvTypes.containsAll (rightEnvTypes))
//					return 0;
				...
				if (cmp == 0) {
					return compareByDefinitionIndex (s1, s2, states);
				}
				return cmp;
			} else {
				EnvironmentType greatestLeftEnvType = envTypeComparator.getGreatestEnvType (leftEnvTypes);
