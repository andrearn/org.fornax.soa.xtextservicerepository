 ...
 *
 */
public class SOAProfileDslScopeProvider extends ImportedNamespaceAwareLocalScopeProvider {
	...
	public IScope getScope(EObject ctx, EReference reference) {
		...
		if (reference==SOAProfileDslPackage.Literals.VERSIONED_TYPE_REF__TYPE && ctx instanceof VersionedTypeRef) {
			...
			return scope_ClassRef_type ((ClassRef) ctx, reference);
			...
			return scope_EnumRef_type ((EnumRef) ctx, reference);
		}
	...
	IScope scope_VersionedTypeRef_type (final VersionedTypeRef ctx, final EReference ref) {
		...
		filter = createVersionFilter(v, ProfileDslElementAccessor.INSTANCE.getVersionedOwner(ctx));
		IScope result = getScope(ctx, ref, filter);
		return result;
	}
	
	IScope scope_ClassRef_type (final ClassRef ctx, final EReference ref) {
		final VersionRef v = ctx.getVersionRef();
		filter = createVersionFilter(v, ProfileDslElementAccessor.INSTANCE.getVersionedOwner(ctx));
		IScope result = getScope(ctx, ref, filter);
		return result;
	...
	IScope scope_EnumRef_type (EnumRef ctx, final EReference ref) {
		final VersionRef v = ctx.getVersionRef();
		filter = createVersionFilter(v, ProfileDslElementAccessor.INSTANCE.getVersionedOwner(ctx));
	...
	protected Object getKey (Notifier context, EReference reference) {
		return Tuples.create (SOAProfileDslScopeProvider.class, context, reference);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected Object getKey (Notifier context, EReference reference, VersionFilter filter) {
		List elements = new ArrayList();
		elements.add (SOAProfileDslScopeProvider.class);
